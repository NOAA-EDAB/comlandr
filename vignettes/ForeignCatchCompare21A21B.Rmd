...
for (ipage in 1:ceiling(length(unique(db21a$Speciesname))/npanels)) {
  p <- db21b %>%
    ...
    ggplot2::geom_line() +
    ggplot2::geom_point(size=0.1) +
    ggforce::facet_wrap_paginate(ggplot2::vars(spCode),nrow=sqrt(npanels),ncol=sqrt(npanels),scales = "free") +
    ggplot2::ylab("Metric Tons (1000's)") +
    ggplot2::scale_x_continuous(limits=c(1960, 2020)) 
  plot(p)
}
...
alldb21a <- readr::read_csv(system.file("extdata","21A.csv",package="comlandr")) %>%
  dplyr::rename(Speciesname = `Species Name`) %>%
  dplyr::rename(MetricTonnes = `Metric Tonnes`) %>%
  dplyr::left_join(.,speciesCodes, by = "Speciesname") %>%
  dplyr::left_join(.,divCodes,by = "Division") %>%
...
all21a <- alldb21a %>%
  ...
  ggplot2::ggplot(data = .,ggplot2::aes(x=Year,y=proportion21a)) +
  ggplot2::geom_line() +
  ggplot2::geom_point(size=0.5) +
...
all21b <- db21b %>%
  ...
  ggplot2::ggplot(data = .,ggplot2::aes(x=Year,y=proportion21b)) +
  ggplot2::geom_line() +
  ggplot2::geom_point(size=0.5) +
  ggplot2::ylab("Proporion of catch")
