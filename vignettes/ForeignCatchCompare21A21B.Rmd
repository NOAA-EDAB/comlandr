...
divCodes <- readr::read_csv(unz(temp,"nafo-21b-2010-16/divisions.txt"),col_names = F) %>%
...
countryCodes <- readr::read_delim(unz(temp, "nafo-21b-2010-16/country.txt"),delim = "\t") %>%
...
speciesCodes <- readr::read_csv(system.file("extdata","species2010_2016.txt",package="comlandr")) %>%
  dplyr::slice(-(1:3)) %>%
...
Data stored on the [NAFO](https://www.nafo.int/Data/Catch-Statistics)'s site is stored in the STATLANT 21 database and is served to the user in one of two formats

The differences are explained in [`comlandr`](https://noaa-edab.github.io/comlandr/articles/ForeignCatch.html) package.
...
p1 <- ggplot2::ggplot(data = divisions) +
  ggplot2::geom_sf(mapping=ggplot2::aes(fill=Divcode),fill="white",color="grey") +
  ggplot2::geom_text(data=divisions,ggplot2::aes(x=X,y=Y,label=Divcode),size=2) +
  ggplot2::ggtitle("NAFO Area Code + stat areas") +
  ggplot2::geom_sf(data=NEUS,mapping=ggplot2::aes(fill=Id),fill="salmon",alpha=.2)


p2 <- ggplot2::ggplot(data = divisions) +
  ggplot2::geom_sf(mapping=ggplot2::aes(fill=Divcode),fill="white",color="grey") +
  ggplot2::geom_text(data=divisions,ggplot2::aes(x=X,y=Y,label=ZONE),size=2) +
  ggplot2::ggtitle("Corresponding Divisions") +
  ggplot2::xlab("") + 
  ggplot2::ylab("") + 
  ggplot2::geom_sf(data=divisions %>% dplyr::filter(Divcode %in% NEUSDivs),mapping=ggplot2::aes(fill=Divcode),fill="salmon",alpha=.2)
...
# read in 21a data
db21a <- readr::read_csv(system.file("extdata","21A.csv",package="comlandr")) %>%
...
# read in 21b data
db21b <- comlandr::get_foreign_data(removeUSA =T) %>%
  tibble::as_tibble()
...
db21a %>%
  ...
  ggplot2::ggplot(.,ggplot2::aes(x = Year, y=Mt/1000)) +
  ggplot2::geom_line() +
  ggplot2::geom_point() +
...
for (ipage in 1:ceiling(length(unique(db21a$Speciesname))/25)) {
  p <- db21a %>%
    dplyr::group_by(Year,Speciesname) %>%
    dplyr::summarise(Mt = sum(MetricTonnes),.groups="drop") %>%
    ggplot2::ggplot(.,ggplot2::aes(x = Year, y=Mt/1000)) +
    ggplot2::geom_line() +
    ggplot2::geom_point() +
    ggforce::facet_wrap_paginate(ggplot2::vars(Speciesname),nrow=5,ncol=5,page=ipage,scales = "free") +
    ggplot2::ylab("Metric Tons (1000's)")
  print(p)
}
```
...
for (ipage in 1:ceiling(length(unique(db21a$Speciesname))/25)) {
  p <- db21b %>%
    ...
    ggplot2::geom_line() +
    ggplot2::geom_point() +
    ggforce::facet_wrap_paginate(ggplot2::vars(Speciesname),nrow=5,ncol=5,scales = "free") +
    ggplot2::ylab("Metric Tons (1000's)")
  plot(p)
}
```
