...
create_comland_EPUs <- function() {
  ...
  # define areas
  statAreaShapefile <- sf::st_read(system.file(
    "extdata",
  ...
  gom$area <- sum(sf::st_area(
    statAreaShapefile %>% dplyr::filter(Id %in% gom$statAreas)
  ))
  units(gom$area) <- "km^2"

  gb$area <- sum(sf::st_area(
    statAreaShapefile %>% dplyr::filter(Id %in% gb$statAreas)
  ))
  units(gb$area) <- "km^2"

  mab$area <- sum(sf::st_area(
    statAreaShapefile %>% dplyr::filter(Id %in% mab$statAreas)
  ))
  units(mab$area) <- "km^2"

  ss$area <- sum(sf::st_area(
    statAreaShapefile %>% dplyr::filter(Id %in% ss$statAreas)
  ))
  units(ss$area) <- "km^2"
