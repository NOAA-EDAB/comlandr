...
assign_unknown <- function(
  comData,
  unkVar,
  knStrata = c('HY', 'QY', 'MONTH', 'NEGEAR', 'TONCL1', 'AREA')
) {
  ...
  for (ivar in 1:length(unkVar)) {
    ...
    for (inum in length(strata.code):1) {
      ...
      num.combos <- data.table::setDT(list(gtools::combinations(
        length(strata.code),
        inum,
        strata.code
      )))
    ...
    if (nrow(unknown) > 0) {
      ...
      for (icom in 1:nrow(combos)) {
        if (nrow(unknown) > 0) {
          ...
          stra.combo <- as.vector(unlist(stra.combo[, ..num.stra.var]))

          #Identify columns that are not part of the stratification
          ext.col <- names(comdata)[which(
            !names(comdata) %in%
              c('NESPP3', 'YEAR', stra.combo, 'VAR', 'SPPLIVMT')
          )]

          #Remove extra columns from known
          known.simple <- data.table::copy(known)
          known.simple <- known.simple[,
            c(names(known)[which(
              names(known) %in%
                ext.col
            )]) := NULL
          ...
          data.table::setnames(
            match,
            c('VAR.x', 'newlivmt', 'newvalue'),
            c('VAR', 'SPPLIVMT', 'SPPVALUE')
          )

          #Append new entries to output
          comdata.out <- data.table::rbindlist(
            list(comdata.out, match),
            use.names = T
          )
          cat(paste(
            'After using',
            length(stra.combo),
            'variables: Unknown records remaining',
            nrow(unknown),
            '\n'
          ))
        }
      ...
      if (nrow(unknown) > 0) {
        ...
        cat(paste(
          'After using 3 year window: Unknown records remaining',
          nrow(unknown),
          '\n'
        ))
        ...
        if (nrow(unknown) > 0) {
          ...
          cat(paste(
            'After using 5 year window: Unknown records remaining',
            nrow(unknown),
            '\n'
          ))
        }
      ...
      if (nrow(unknown) > 0) {
        ...
        cat(paste(
          'After using only species: Unknown records remaining',
          nrow(unknown),
          '\n'
        ))
      }
    ...
    comdata.out <- data.table::rbindlist(
      list(comdata.out, unknown[, ID := NULL]),
      use.names = T
    )
